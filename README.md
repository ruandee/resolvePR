...
ResolvePR reviews every pull request for security vulnerabilities with Claude and posts the results as inline comments, a summary, and a check run — pinned to the exact changed line, with a click-to-apply fix. It only sends the functions a PR actually touched, so the review is fast, cheap, and low-noise.
...
3. Add `.github/workflows/resolvepr.yml`:
...
jobs:
  resolvepr:
    ...
    steps:
      - uses: ruandee/resolvepr@v1
        with:
          anthropic_api_key: ${{ secrets.ANTHROPIC_API_KEY }}
```

That's it. The next PR gets a **ResolvePR** check run, inline comments with `suggestion` blocks, and a severity summary.
...
- PRs from forks get a read-only `GITHUB_TOKEN`; ResolvePR logs the failed comment posts and still prints the findings in the job log, the job summary, and as workflow annotations.
...
One binary, three modes. Build it with `go build ./cmd/resolvepr` (see [Development](#development) for the CGO requirement).
...
resolvepr scan --repo acme/api --pr 142 --post --token $GITHUB_TOKEN

# Review your local branch against main, before opening a PR
resolvepr scan --local --base main
...
resolvepr scan --local --base main --json --fail-on high
```
...
| Flag | Meaning |
|---|---|
| `--api-key` / `--model` | Override `ANTHROPIC_API_KEY` / `RESOLVEPR_MODEL` (default model `claude-opus-5`). |
...
`resolvepr action` is what `action.yml` runs; it reads the GitHub Actions environment (`GITHUB_EVENT_PATH`, `INPUT_*`) and needs no flags. `resolvepr serve` is the self-hosted webhook server (below). `resolvepr version` prints the version.
...
ResolvePR extracts just the `transfer` function, understands it's Go, and flags the SQL injection on the exact diff line — not across a 500-line file.
...
If you would rather run ResolvePR as a GitHub App than as an Action — for org-wide coverage and the dashboard's live findings feed — `resolvepr serve` is the long-running webhook server:

```bash
# Required: WEBHOOK_SECRET, GITHUB_APP_ID, GITHUB_PRIVATE_KEY_PATH (or GITHUB_PRIVATE_KEY), ANTHROPIC_API_KEY
# Optional: DATABASE_URL (Neon/Postgres; in-memory store if unset), PORT (8080), RESOLVEPR_MODEL
resolvepr serve
...
The dashboard that reads from it lives in [`resolvepr-dashboard/`](resolvepr-dashboard/) (Next.js; see its README). Its `/demo` page replays a fixture produced by `resolvepr scan --fixture-out`.
...
```
cmd/resolvepr/         scan · action · serve · version
...
ResolvePR ships no key and runs no service of its own. Every review is billed to the `ANTHROPIC_API_KEY` you provide, at Anthropic's normal rates; the system prompt is cached, so a typical PR costs a few cents. The key is read only by the process you run (the Action container, your shell, or your server) and is sent only to `api.anthropic.com`.
