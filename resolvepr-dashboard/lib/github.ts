...
export const BOT_LOGIN = 'resolvepr'
...
export function buildCommentBody(f: Finding): string {
  return (
    ...
    '```suggestion\n' + f.fix_patch + '\n```' +
    `\n\n_ResolvePR · confidence ${confidencePct(f)}%_`
  )
}
...
export const NO_ISSUES_SUMMARY = 'All scanned chunks clean. ResolvePR found no security issues.'
...
export function buildSummary(findings: Finding[]): string {
  ...
  return `## ResolvePR found ${findings.length} issues
...
export function checkTitle(findings: Finding[]): string {
  return findings.length === 0 ? 'ResolvePR — no issues found' : `ResolvePR found ${findings.length} issues`
}
